...
for league in leagues:
    for method in methods:
        df = pd.read_csv(f"results/{method}_roi/{league}_seasonal_binned_roi.csv")
        ...
        for b in sorted(df_clean['bin'].unique()):
            ...
            plt.title(f'{league.upper()} ROI by Season (Bin {b}) ({method})')
            plt.ylabel('ROI')
            plt.xlabel('Season')
            ...
            plt.tight_layout()
            plt.savefig(f"figures/{method}_roi/{league}_bin{b}_seasonal_roi.png")
            plt.close()
