...
for league in leagues:
    ...
    def compute_brier_scores(df_subset, period):
        return [
            {'Prediction_Method': f'{league}_{period}_bradley_terry', 'Brier_Score': ((df_subset['bradley_terry_prediction'] - df_subset['result']) ** 2).dropna().mean()}
        ]
...
brier_scores_df.to_csv('results/bradley_terry/bradley_terry_half_full_brier.csv', index=False)
